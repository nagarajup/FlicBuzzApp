    ...
    public void launchPurchaseFlow(Activity act, String sku, String itemType, List<String> oldSkus,
        ...
        try {
            ...
            if (response != BILLING_RESPONSE_RESULT_OK) {
                ...
                if(getResponseDesc(response).contains("Item Already Owned")){
                    Utility.purchaseFlag=true;
                    Toast.makeText(act,"Item Already Owned", Toast.LENGTH_SHORT).show();
                }
                flagEndAsync();
           ...
           /* if(getResponseDesc(response).contains("Item Already Owned")){
                CommonObject.purchaseFlag=true;
                Toast.makeText(act,"Item Already Owned",Toast.LENGTH_SHORT).show();
                flagEndAsync();
                result = new IabResult(response, "Unable to buy item");
                if (listener != null) listener.onIabPurchaseFinished(result, null);
